...
- **Float constants are emitted as host IEEE words, not PDP-11 float
  format** (the four `((unsigned short*)&fvalue)[i]` words). Integer and
  long paths are correct; PDP-11 floating-point support is a later task.
